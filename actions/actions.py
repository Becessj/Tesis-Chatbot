...
class ActionDepedida(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        dispatcher.utter_message(template="utter_goodbye", buttons = [
            {"payload": "/bueno", "title": "Bueno"},
            {"payload": "/malo", "title": "Malo"},
        ])
        return []
...
class ActionConsultarTramites(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        año = tracker.get_slot('año')
        codigo = tracker.get_slot('codigo')

        print("Los datos que han llegado son:  " + año + " " +codigo)

        the_data = {"anio": año, "codigo": codigo}
        ...
        if len(tramites) == 0:
            ...
            dispatcher.utter_message(text=f"Tienes los siguientes tramites durante el año {año} \n \n")
            for k in range(len(tramites)):
                dispatcher.utter_message(text=f"{k+1}.-{str(tramites[k])}")
        result = [SlotSet("año", año),SlotSet("codigo", codigo)]
        return result
...
class ActionConsultarDescripcionTramite(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        if(Resultado.size>0):
            ...
            dispatcher.utter_message(text=f"El trámite {Nombre} \n\n es un {Descripcion}")
        else:
...
class ActionConsultarMontoTramite(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        if(Resultado.size>0):
            ...
            dispatcher.utter_message(text=f"El costo del trámite {Nombre} \n\n es de S/.{Monto}")
        else:
            dispatcher.utter_message(template="utter_fallback")
        return []
